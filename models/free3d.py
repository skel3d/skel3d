...
class Free3DDiffusion(LatentDiffusion):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False, cond_views=1,
        ...
        skels = rearrange(batch['skeletons'].to(memory_format=torch.contiguous_format).float(), 'b v h w c -> b v c h w').to(self.device)
        ...
        # skeleton        
        tgt_skel = rearrange(skels[:, tgt_index, ...], 'b v ... -> (b v) ...')
        skels_z = self.encode_first_stage(tgt_skel).mode().detach()
        ...
        if return_original_cond:
            out.extend([src_img,skels[:, 0, ...],tgt_skel])
        return out
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1.0, return_keys=None,
        ...
        log = dict()
        z, c_cond, x, xrec, xc , xs, skels= self.get_input(batch, self.first_stage_key,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc.detach().cpu()
                log["time0skel"] = xs.detach().cpu()
                log["timegt"] = skels.detach().cpu()
            if ismap(xc):
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            with ema_scope("Sampling with classifier-free guidance"):
                samples_cfg, _ = self.sample_log(cond=c_cond, batch_size=N, ddim=use_ddim,
    ...
    def get_unconditional_conditioning(self, batch_size, null_label=None, image_size=512):
        ...
        img_size = 32
        cond["c_skel"] = torch.zeros([batch_size, 4, img_size, img_size]).to(self.device)#torch.zeros([batch_size, self.concat_channel, image_size // 8, image_size // 8]).to(self.device)
